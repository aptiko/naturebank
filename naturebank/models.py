...
class Biotope(models.Model):
    ...
    site_code = models.CharField(
        ...
        help_text=(
            'Ο Κωδικός πρέπει να έχει πρόθεμα ένα εκ των "ΑΤ", '
            '"AΘ", "AG", "AB" ή "GR".'
        ),
    )
    ...
    owner = models.ManyToManyField(
        "OwnerOption", verbose_name="Ιδιοκτησία", help_text="Κατηγορίες ιδιοκτησίας."
    )

    # Sitetype relation through intermediate table.
    site_type = models.ManyToManyField(
        "SiteTypeOption", verbose_name="Τύπος τοπίου", help_text="Τύποι τοπίων."
    )

    # Climate relation through intermediate table.
    climate = models.ManyToManyField(
        "ClimateOption", verbose_name="Κλίμα", help_text="Τύποι κλίματος."
    )

    # Ecological value relation through intermediate table.
    ecological_value = models.ManyToManyField(
    ...
    social_value = models.ManyToManyField(
        "SocialValueOption",
    ...
    cultural_value = models.ManyToManyField(
        "CulturalValueOption",
    ...
    threat = models.ManyToManyField(
        "ThreatOption", verbose_name="Απειλές", help_text="Κατηγορίες απειλών"
    )

    # Human activity m2m
    human_activity = models.ManyToManyField(
        "HumanActivityOption",
    ...
    # Habitation m2m
    habitation = models.ManyToManyField(
        "HabitationOption",
    ...
    main_char_biotopos = models.BooleanField(
        default=False, verbose_name="Βιότοπος", help_text="Είναι Βιότοπος;"
    )
    main_char_natural = models.BooleanField(
        default=False, verbose_name="Φυσικό Τοπίο", help_text="Είναι Φυσικό Τοπίο;"
    )
    main_char_built = models.BooleanField(
        default=False, verbose_name="Δομημένο Τοπίο", help_text="Είναι Δομημένο Τοπίο;"
    )

    # Geographical fields (BIOLOCATION).
    geo_code = models.ForeignKey(
        "GeoCodeOption",
        null=True,
        blank=True,
    ...
    document = models.TextField(
        ...
        help_text=(
            "Πηγές, βιβλιογραφία και αναφορές " "από όπου αντλήθηκαν τα δεδομένα."
        ),
    )
    ...
    measures_need = models.TextField(
        blank=True,
    ...
    intro_text = models.TextField(
        blank=True, verbose_name="Εισαγωγικό Κείμενο", help_text="Εισαγωγικό κείμενο"
    )

    # Social Reaction
    social_reaction = models.ForeignKey(
    ...
    social_reaction_text = models.TextField(
        blank=True,
    ...
    ownership_text = models.TextField(
        blank=True,
    ...
    landvalue = models.FloatField(
        ...
        help_text=(
            "Η μέγιστη αξία γης του τόπου (Οι παλιές εγγραφές είναι σε" " δρχ./στρέμμα)"
        ),
    )

    species_text = models.TextField(
        blank=True,
    ...
    gis_perimeter = models.FloatField(
        null=True, blank=True, verbose_name="Περίμετρος m"
    )
    gis_sitecode = models.CharField(
        null=True, blank=True, max_length=9, verbose_name="Κωδικός από GIS (sitecode)"
    )
    gis_zorder = models.IntegerField(
        null=True, blank=True, verbose_name="Σειρά στρώσης καθ΄ύψος"
    )
    ...
    def __str__(self):
        if self.site_name_gr:
            return "%s" % self.site_name_gr
        else:
            return "%s | N/A" % (self.site_code,)
...
class BiotopeImage(models.Model):
    ...
    remarks = models.TextField(
        blank=True, verbose_name="Σχόλια", help_text="Σχόλια για την εικόνα"
    )
...
class HabitationOption(models.Model):
    ...
    name = models.CharField(
        max_length=390, blank=True, help_text="Το όνομα του ενδιαιτήματος"
    )
    ...
    def __str__(self):
        if self.name:
            ...
            return "%s | N/A" % (self.abbreviation,)
...
class TrendPopOption(models.Model):
    ...
    # Core Fields
    name = models.CharField(
        max_length=300, blank=True, help_text="Όνομα τάσης πληθυσμού"
    )
...
class HumanActivityOption(models.Model):
    ...
    name = models.CharField(
        max_length=390, blank=True, help_text="Το όνομα της ανθρώπινης δραστηριότητας"
    )
...
class ThreatOption(models.Model):
    ...
    abbreviation = models.CharField(
        max_length=30, blank=False, unique=True, help_text="Συντομογραφία της απειλής"
    )
    name = models.CharField(
        max_length=390, blank=True, help_text="Το όνομα της απειλής"
    )
...
class CulturalValueOption(models.Model):
    ...
    name = models.CharField(
        max_length=390, blank=True, help_text="Το όνομα της αισθητικής αξίας"
    )
...
class EcologicalValueOption(models.Model):
    ...
    name = models.CharField(
        max_length=390, blank=True, help_text="Το όνομα της οικολογικής αξίας"
    )
...
class ClimateOption(models.Model):
    ...
    name = models.CharField(
        max_length=390, blank=True, help_text="Το όνομα του τύπου κλίματος"
    )
...
class SiteTypeOption(models.Model):
    ...
    name = models.CharField(max_length=390, blank=True, help_text="Το όνομα του τύπου")
...
class OwnerOption(models.Model):
    ...
    # Core Fields
    name = models.CharField(max_length=300, blank=True, help_text="Κατάσταση βιότοπου")
...
class GeoCodeOption(models.Model):
    ...
    name = models.CharField(
        max_length=300, blank=True, help_text="Το όνομα της γεωγραφικής περιοχής"
    )
...
class TrendOption(models.Model):
    ...
    # Core Fields
    name = models.CharField(
        max_length=300, blank=True, help_text="Τάση κατάστασης βιότοπου"
    )
...
class KnowledgeOption(models.Model):
    ...
    # Core Fields
    name = models.CharField(max_length=300, blank=True, help_text="Κατάσταση γνώσης")
...
class SocialReactionOption(models.Model):
    ...
    # Core Fields
    name = models.CharField(
        max_length=300, blank=True, help_text="Περιγραφή κοινωνικής αντίδρασης"
    )
...
class ConservationOption(models.Model):
    ...
    # Core Fields
    name = models.CharField(
        max_length=300, blank=True, help_text="Περιγραφή προτεραιότητας προστασίας"
    )
...
class WideArea(models.Model):
    ...
    # Core Fields
    wide_area_name = models.CharField(
        max_length=150, blank=True, help_text="Όνομα ευρύτερης γεωγραφικής περιοχής."
    )
    ...
    def __str__(self):
        if self.wide_area_name:
            return "%s" % self.wide_area_name
        else:
            return "%s | N/A" % (self.id,)
...
class AbandonmentOption(models.Model):
    ...
    # Core Fields
    name = models.CharField(
        max_length=300, blank=True, help_text="Τίτλος επιπέδου εγκατάλειψης."
    )
...
class Species(models.Model):
    ...
    # Core fields
    species_code = models.IntegerField(
        null=False,
        unique=True,
    ...
    species_name_gr = models.CharField(
        max_length=720,
        blank=True,
    ...
    respondent = models.TextField(
        blank=True, verbose_name="Στοιχεία Καταχωρητή", help_text="Στοιχεία καταχωρητή"
    )
    ...
    # Conservation
    conservation_gr = models.ForeignKey(
    ...
    threat = models.TextField(
        blank=True, verbose_name="Σχόλια", help_text="Περιγραφή απειλών και κινδύνων"
    )

    # Attributes (Boolean)
    category_ende = models.BooleanField(
        blank=True, default=False, verbose_name="Ενδημικό", help_text="Είναι ενδημικό"
    )
    category_migr = models.BooleanField(
        blank=True,
        default=False,
        verbose_name="Μεταναστευτικό",
        help_text="Είναι μεταναστευτικό",
    )
    category_bree = models.BooleanField(
        blank=True,
        default=False,
        verbose_name="Αναπαραγόμενο",
        help_text="Είναι αναπαραγόμενο",
    )
    category_resi = models.BooleanField(
        blank=True,
        default=False,
        verbose_name="Μόνιμος Κάτοικος",
        help_text="Κατοικεί μόνιμα",
    )
    category_intr = models.BooleanField(
        blank=True, default=False, verbose_name="Εισαχθέν", help_text="Είναι εισαχθέν"
    )
    category = models.TextField(
        blank=True, verbose_name="Σχόλια", help_text="Σχόλια για τα γνωρίσματα"
    )

    # Exploitation (Boolean)
    exploit_hunt = models.BooleanField(
        blank=True,
        default=False,
        verbose_name="Κυνήγι",
        help_text="Εκμετάλλευση από το κυνήγι",
    )
    exploit_fish = models.BooleanField(
        blank=True,
        default=False,
        verbose_name="Αλιεία",
        help_text="Εκμετάλλευση από το αλιεία",
    )
    exploit_coll = models.BooleanField(
        blank=True,
        default=False,
        verbose_name="Συλλογή",
        help_text="Εκμετάλλευση από τη σύλλογη",
    )
    exploit_logg = models.BooleanField(
        blank=True,
        default=False,
        verbose_name="Υλοτομία",
        help_text="Εκμετάλλευση από την υλοτομία",
    )
    exploit_graz = models.BooleanField(
        blank=True,
        default=False,
        verbose_name="Βοσκή",
...
class SpeciesCategoryOption(models.Model):
    ...
    name = models.CharField(
        max_length=300, blank=True, help_text="Η ονομασία του είδους"
    )
...
class SpeciesPlantKindOption(models.Model):
    ...
    name = models.CharField(
        max_length=300, blank=True, help_text="Η ονομασία αυτής της κατηγορίας φυτών"
    )
...
class Settlement(models.Model):
    ...
    name = models.CharField(max_length=50, blank=True, help_text="Ονομασία οικισμού")
